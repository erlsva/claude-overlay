...
export function measureMediaUrl(url: string, kind: "image" | "video"): Promise<{ width: number; height: number } | null> {
  return new Promise((resolve) => {
    const finish = (width: number, height: number) => {
      ...
      resolve({ width: Math.max(1, Math.round(width * scale)), height: Math.max(1, Math.round(height * scale)) });
    };
