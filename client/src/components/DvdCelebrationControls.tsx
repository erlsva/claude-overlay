...
interface Props {
  ...
  uploading: boolean;
  onChange: (settings: DvdCelebrationSettings) => void;
  onSoundUpload: (event: React.ChangeEvent<HTMLInputElement>) => void;
}
...
export function DvdCelebrationControls({ settings, uploading, onChange, onSoundUpload }: Props) {
  ...
  const preview = () => {
    ...
    [659.25, 783.99, 1046.5].forEach((frequency, index) => {
      ...
      gain.gain.setValueAtTime(Math.max(0.001, settings.volume * 0.22), start);
  ...
  return (
    <div className="dvd-inline-settings">
      <div className="dvd-inline-settings__sound">
        <span>Corner sound</span>
        <strong title={settings.soundUrl ?? undefined}>
          {settings.soundUrl ? getFileLabel(settings.soundUrl) : "Built-in chime"}
        </strong>
        <button
          className="ui-icon-button"
          onClick={preview}
          title={playing ? "Stop the preview" : "Preview corner sound on this dashboard"}
        >
          {playing ? <Square size={11} fill="currentColor" /> : <Headphones size={13} />}
        </button>
      </div>
      <label>
        <span>Volume</span>
      ...
      <label>
        ...
        <select
          ...
          onChange={(event) => {
            onChange({
              ...settings,
              counterPosition: event.target.value as DvdCelebrationSettings["counterPosition"],
            });
            toast.success(
              `DVD counter moved to ${event.target.options[event.target.selectedIndex].text.toLowerCase()}`,
            );
          }}
          title="Choose the corner counter position"
        >
      ...
      <div className="dvd-inline-settings__actions">
        <label className="ui-button ui-button--compact" title="Upload a DVD corner sound">
          <Upload size={12} />
          {uploading ? "Uploading…" : settings.soundUrl ? "Replace sound" : "Upload sound"}
          <input
            hidden
            type="file"
        ...
        <button
          ...
          disabled={!settings.soundUrl}
          onClick={() => {
            onChange({ ...settings, soundUrl: null });
