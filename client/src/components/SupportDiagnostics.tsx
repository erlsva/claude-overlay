...
export function SupportDiagnostics({ snapshot }: { snapshot: SnapshotInput }) {
  ...
  const report = useMemo(
    () => buildDiagnosticReport({ ...snapshot, notifications }),
  ...
  return (
    <>
      <button
        className="ui-icon-button canvas-corner-button support-button"
        onClick={() => setOpen(true)}
        title="Open notification history and support diagnostics"
      ...
      {presence.mounted && (
        <div
          className="support-backdrop motion-backdrop"
          data-state={presence.state}
          onMouseDown={(event) => event.target === event.currentTarget && setOpen(false)}
        >
          <section
            className="support-dialog motion-dialog"
            data-state={presence.state}
            role="dialog"
            aria-modal="true"
          ...
          >
            <header>
              <span>
                <strong id="support-title">Support & notifications</strong>
                <small>
                  Everything needed to describe a problem without exposing private credentials
                </small>
              </span>
              <button
            ...
            <div className="support-summary">
              ...
              <Status
                label="Overlay"
                ok={snapshot.overlayConnected}
                detail={snapshot.overlayConnected ? String(snapshot.overlayCount) : undefined}
              />
            ...
            <div className="support-history-header">
              <span>
                ...
                <small>Kept for this browser session</small>
              </span>
              <button
                className="ui-button ui-button--compact"
                onClick={clearNotifications}
                disabled={!notifications.length}
                title="Clear this browser's notification history"
              >
            ...
            <div className="support-history">
              {[...notifications].reverse().map((item) => {
                ...
                return (
                  <div
                    key={item.id}
                    className={`support-history-item support-history-item--${item.kind}`}
                  >
                    <Icon size={15} />
                    <span>
            ...
            <footer>
              <span>Copy this report and send it with a screenshot when asking for help.</span>
              <button className="ui-button studio-primary" onClick={copy}>
...
function Status({ label, ok, detail }: { label: string; ok: boolean; detail?: string }) {
  return (
    <span
      className={ok ? "support-status support-status--ok" : "support-status support-status--bad"}
    >
      <i /> {label}: {ok ? "online" : "offline"}
      {detail ? ` · ${detail}` : ""}
    </span>
  );
}
