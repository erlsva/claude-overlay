...
export function estimateTextElementSize(config: TextConfig) {
  ...
  const wrappedLines = lines.reduce(
    (total, line) =>
      total + Math.max(1, Math.ceil((line.length * characterWidth) / Math.max(1, width - 36))),
    0,
  );
  const height = Math.min(
    800,
    Math.max(70, wrappedLines * config.fontSize * config.lineHeight + 28),
  );
...
export function TextDialog({ initial, onConfirm: confirmNow, onClose: closeNow }: TextDialogProps) {
  ...
  const update = <K extends keyof TextConfig>(key: K, value: TextConfig[K]) =>
  ...
  return (
    <div
      className="text-editor-backdrop motion-backdrop"
      data-state={closing ? "closed" : "open"}
      role="presentation"
      onMouseDown={(event) => event.target === event.currentTarget && onClose()}
    >
      <section
        className="text-editor-dialog motion-dialog"
        data-state={closing ? "closed" : "open"}
        role="dialog"
        aria-modal="true"
      ...
      >
        <header className="text-editor-header">
          <span className="text-editor-heading-icon">
            <Type size={17} />
          </span>
          <span>
            <strong id="text-editor-title">{initial ? "Edit text" : "Add text"}</strong>
            <small>Style a text layer for the dashboard and overlay</small>
          </span>
          <button
        ...
        <div
          ...
          style={{
            ...
            WebkitTextStroke: config.strokeWidth
              ? `${Math.min(config.strokeWidth, 4)}px ${config.strokeColor}`
              : undefined,
        ...
        >
          {config.text || "Your text will appear here"}
        </div>
        ...
        <label className="text-editor-field text-editor-field--wide">
          ...
          <textarea
            value={config.text}
            onChange={(event) => update("text", event.target.value.slice(0, 9_500))}
            placeholder="Type something for the stream…"
            rows={3}
            autoFocus
          />
        ...
        <div className="text-editor-grid">
          <label className="text-editor-field text-editor-field--font">
            <span>Font</span>
            <select
          ...
          <label className="text-editor-field">
            <span>Alignment</span>
            <span className="text-editor-segmented">
              {(["left", "center", "right"] as TextAlignment[]).map((alignment) => {
                ...
                return (
                  <button
                    type="button"
                    key={alignment}
                    className={config.textAlign === alignment ? "active" : ""}
        ...
        <div className="text-editor-range-card">
          <label>
            ...
            <input
              ...
              value={config.fontSize}
              onChange={(event) =>
                update("fontSize", Math.min(400, Math.max(8, Number(event.target.value))))
              }
            />
            <output>px</output>
          </label>
          <div className="text-editor-presets">
            {SIZE_PRESETS.map((size) => (
              <button
                type="button"
                key={size}
                className={config.fontSize === size ? "active" : ""}
          ...
          <label>
            ...
            />
            <output>{config.lineHeight.toFixed(2)}</output>
          </label>
          <label>
            ...
            />
            <output>{config.letterSpacing}px</output>
          </label>
        </div>

        <div className="text-editor-effects">
          <div className="text-editor-effect-row">
            <label className="text-editor-toggle">
              <input
                type="checkbox"
                checked={config.strokeWidth > 0}
                onChange={(event) => update("strokeWidth", event.target.checked ? 2 : 0)}
              />
              <span>Outline</span>
            </label>
            {config.strokeWidth > 0 && (
              <>
                ...
                />
                <output>{config.strokeWidth}px</output>
                <input
                  type="color"
        ...
        <footer className="text-editor-actions">
          <span>Esc to close · Ctrl+Enter to save</span>
          ...
          <button
            ...
            type="button"
            disabled={!valid}
            onClick={() => valid && onConfirm(config)}
          >
            {initial ? "Save changes" : "Add to overlay"}
          </button>
        </footer>
      </section>
    </div>
