...
export function WhitelistPanel({ open, onClose, isOwner, isAdmin }: WhitelistPanelProps) {
  ...
  const handleToggleAdmin = async (username: string, isAdmin: boolean) => {
    try {
      ...
      if (!response.ok) { toast.error(`Could not update ${username}'s role`); return; }
      toast.success(`${username} is ${isAdmin ? 'no longer a super moderator' : 'now a super moderator'}`);
      fetchList();
  ...
  return (
    <div className={`whitelist-backdrop${open ? ' whitelist-backdrop--open' : ''}`} aria-hidden={!open} onClick={onClose}>
      <div className="whitelist-panel" style={{ background: 'var(--bg-panel)', borderLeft: '1px solid var(--line)', display: 'flex', flexDirection: 'column', boxShadow: '0 0 32px rgba(0,0,0,0.55)' }} onClick={(e) => e.stopPropagation()}>
        <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', padding: '0 12px 0 16px', height: 56, borderBottom: '1px solid var(--line)' }}>
          <h2 style={{ margin: 0, fontSize: 15, fontWeight: 600, color: 'var(--text-primary)' }}>Whitelist</h2>
          <button className="ui-icon-button" title="Close whitelist settings" aria-label="Close whitelist settings" onClick={onClose} style={{ background: 'none', border: '1px solid transparent', color: 'var(--text-muted)', cursor: 'pointer', display: 'flex', alignItems: 'center' }}><X size={18} /></button>
        </div>

        {isAdmin && (
          <div style={{ padding: 16, borderBottom: '1px solid var(--line)' }}>
            <p style={{ margin: '0 0 8px', fontSize: 12, fontWeight: 500, color: 'var(--text-muted)' }}>Add a Twitch username</p>
            <div style={{ display: 'flex', gap: 8 }}>
              <input value={input} onChange={(e) => setInput(e.target.value)} onKeyDown={(e) => e.key === 'Enter' && handleAdd()}
                placeholder="username" style={{ flex: 1, minWidth: 0, height: 32, background: 'var(--bg-sunken)', border: '1px solid var(--line-strong)', borderRadius: 6, color: 'var(--text-primary)', fontSize: 13, padding: '0 12px', outline: 'none' }} />
              <button className="ui-button studio-primary" onClick={handleAdd} disabled={loading || !input.trim()}
                style={{ width: 'auto' }}>
                {loading ? '…' : 'Add'}
              </button>
            </div>
        ...
        <div style={{ flex: 1, overflowY: 'auto' }}>
          {list.length === 0
            ? <p style={{ padding: 16, fontSize: 12, color: 'var(--text-muted)', textAlign: 'center' }}>No users whitelisted yet</p>
            : list.map((entry) => (
              <div key={entry.username} className="whitelist-row">
                <div className="whitelist-row__who">
                  ...
                  <span className="whitelist-row__added">Added {new Date(entry.added_at).toLocaleDateString()}{entry.added_by ? ` by ${entry.added_by}` : ''}</span>
                </div>
                {isAdmin && (
                  <div className="whitelist-row__actions">
                    {isOwner && (
                      <button
                        ...
                        aria-label={entry.isAdmin ? `Remove ${entry.username} as super moderator` : `Make ${entry.username} a super moderator`}
                        title={entry.isAdmin ? "Remove super moderator" : "Make super moderator"}
                      >
                        <Star size={13} fill={entry.isAdmin ? "currentColor" : "none"} />
                      </button>
                    )}
                    <button className="ui-button ui-button--compact ui-button--quiet-danger" onClick={() => handleRemove(entry.username)}>
                      Remove
                    </button>
                  </div>
                )}
