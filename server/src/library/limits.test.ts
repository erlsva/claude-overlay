...
test("rejects unsupported, empty and oversized files", () => {
  assert.match(
    checkLibraryUpload({ mime: "application/zip", size: 10, usedBytes: 0 })!,
    /not supported/,
  );
  assert.match(checkLibraryUpload({ mime: "image/png", size: 0, usedBytes: 0 })!, /empty/);
  assert.match(
    checkLibraryUpload({ mime: "image/png", size: MAX_FILE_BYTES + 1, usedBytes: 0 })!,
    /at most 25 MB/,
  );
});
...
test("rejects a file that would overflow the total budget, but allows one that fits exactly", () => {
  ...
  assert.match(
    checkLibraryUpload({ mime: "image/png", size: 2_000, usedBytes: nearlyFull })!,
    /library is full/,
  );
