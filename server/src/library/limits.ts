...
export function checkLibraryUpload(input: { mime: string; size: number; usedBytes: number }): string | null {
  if (!allowedMimeTypes.has(input.mime)) return "That file type is not supported.";
  if (input.size <= 0) return "That file is empty.";
  if (input.size > MAX_FILE_BYTES) return `Files in the library can be at most ${megabytes(MAX_FILE_BYTES)} MB.`;
