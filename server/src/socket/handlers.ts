...
function historyStatus(store: CanvasStore) {
...
export function registerSocketHandlers(
  ...
  socket.on("element:update", ({ id, changes }) => {
    ...
    const affectedGroupId = typeof changes.groupId === "string" ? changes.groupId : element.groupId;
    const transformKeys = new Set(["x", "y", "width", "height", "rotation", "scaleX", "scaleY"]);
    const isGroupTransform =
      !!affectedGroupId &&
      changeKeys.length > 0 &&
      changeKeys.every((key) => transformKeys.has(key));
    record(
  ...
  socket.on("chat-emote:settings", (settings) => {
    if (
      ...
      !Number.isFinite(settings.nameFontSize) ||
      settings.nameFontSize < 9 ||
      settings.nameFontSize > 32 ||
      !["walls", "floor", "parade", "corners"].includes(settings.motion) ||
      !["left", "right"].includes(settings.direction) ||
      !Number.isFinite(settings.gravity) ||
      settings.gravity < 100 ||
      settings.gravity > 2400 ||
      ...
      settings.lifetimeSeconds < 2 ||
      settings.lifetimeSeconds > 120 ||
      ...
      settings.blacklist.some(
        (name) => typeof name !== "string" || !/^[a-z0-9_]{1,25}$/i.test(name),
      ) ||
      !Array.isArray(settings.additionalEmotes) ||
      settings.additionalEmotes.length > 100 ||
      settings.additionalEmotes.some(
        (name) => typeof name !== "string" || !/^[a-z0-9_]{1,64}$/i.test(name),
      ) ||
      (settings.blockedEmotes !== undefined &&
        (!Array.isArray(settings.blockedEmotes) ||
          settings.blockedEmotes.length > 100 ||
          settings.blockedEmotes.some(
            (name) => typeof name !== "string" || !/^\S{1,64}$/.test(name),
          )))
    )
      return;
  ...
  socket.on("preset:save", async ({ id, name, elementIds }) => {
    if (
      ...
      return;
    const elements = canvasState.elements.filter((element) => elementIds.includes(element.id));
    if (!elements.length) return;
    store.presets = [
      ...store.presets.filter((item) => item.id !== id),
      { id, name: name.trim(), elements: clone(elements), createdAt: new Date().toISOString() },
    ].slice(-100);
  ...
  socket.on("preset:load", ({ id }) => {
    ...
    const copies = preset.elements.map((element) => ({
      ...clone(element),
      id: randomUUID(),
      x: element.x + 32,
      y: element.y + 32,
      groupId: element.groupId
        ? (groups.get(element.groupId) ??
          (() => {
            const value = randomUUID();
            groups.set(element.groupId!, value);
            return value;
          })())
        : undefined,
    }));
    canvasState.elements.push(...copies);
  ...
  socket.on("sound:save", async (item) => {
    if (
      ...
      return;
    store.sounds = [...store.sounds.filter((sound) => sound.id !== item.id), item].slice(-100);
  ...
  socket.on("trigger:save", async (trigger) => {
    ...
    store.triggers = [...store.triggers.filter((item) => item.id !== trigger.id), trigger].slice(
      -100,
    );
...
function validTriggerStep(value: any): boolean {
  ...
  return (
    ...
    (value.delaySeconds === undefined ||
      (Number.isFinite(value.delaySeconds) &&
        value.delaySeconds >= 0 &&
        value.delaySeconds <= 3600)) &&
...
function validTrigger(value: any): boolean {
  ...
  return (
    ...
    value.cooldownSeconds <= 86400 &&
    (value.match === undefined || (typeof value.match === "string" && value.match.length <= 100)) &&
    (value.minimum === undefined ||
      (Number.isFinite(value.minimum) && value.minimum >= 0 && value.minimum <= 10_000_000)) &&
