...
export function validElement(element: CanvasElement): boolean {
  return (
    !!element &&
    boundedString(element.id, 100) &&
    MEDIA_TYPES.has(element.type) &&
    ...
    finite(element.rotation) &&
    finite(element.scaleX) &&
    finite(element.scaleY) &&
    typeof element.visible === "boolean" &&
    finite(element.zIndex) &&
    ...
    [
      element.dvdStartedAt,
      element.dvdStartX,
      element.dvdStartY,
      element.dvdVelocityX,
      element.dvdVelocityY,
    ...
    (element.effectDurationMs === undefined ||
      (finite(element.effectDurationMs) &&
        element.effectDurationMs >= 150 &&
        element.effectDurationMs <= 10_000))
  );
}
...
export function validElementUpdate(changes: Partial<CanvasElement>): boolean {
  ...
  for (const key of ["width", "height"] as const) {
    if (
      ...
      (!finite(candidate[key]) ||
        ...
        (candidate[key] as number) > 10_000)
    )
      return false;
  }
  ...
  if (
    ...
    (!finite(candidate.opacity) ||
      (candidate.opacity as number) < 0 ||
      (candidate.opacity as number) > 1)
  )
    return false;
  if ("enterAnimation" in candidate && !ANIMATIONS.has(candidate.enterAnimation as string))
    return false;
  if ("exitAnimation" in candidate && !ANIMATIONS.has(candidate.exitAnimation as string))
    return false;
  if ("effectAnimation" in candidate && !EFFECT_ANIMATIONS.has(candidate.effectAnimation as string))
    return false;
  ...
  if (
    "effectDurationMs" in candidate &&
    (!finite(candidate.effectDurationMs) ||
      (candidate.effectDurationMs as number) < 150 ||
      (candidate.effectDurationMs as number) > 10_000)
  )
    return false;
  if (
    "mediaVolume" in candidate &&
    (!finite(candidate.mediaVolume) ||
      (candidate.mediaVolume as number) < 0 ||
      (candidate.mediaVolume as number) > 1)
  )
    return false;
  if (
    ...
    !boundedString(candidate.groupId, 100)
  )
    return false;
  if (
    "groupName" in candidate &&
    candidate.groupName !== undefined &&
    !boundedString(candidate.groupName, 80)
  )
    return false;
  if (
    "displayName" in candidate &&
    candidate.displayName !== undefined &&
    !boundedString(candidate.displayName, 120)
  )
    return false;
  return true;
}
...
export function validStroke(stroke: DrawStroke): boolean {
  ...
  if (
    stroke.tool !== undefined &&
    !["pen", "eraser", "fill", "line", "arrow", "rectangle", "ellipse"].includes(stroke.tool)
  )
    return false;
  if (
    stroke.opacity !== undefined &&
    (!finite(stroke.opacity) || stroke.opacity < 0.05 || stroke.opacity > 1)
  )
    return false;
  if (
    stroke.fillTolerance !== undefined &&
    (!finite(stroke.fillTolerance) || stroke.fillTolerance < 0 || stroke.fillTolerance > 255)
  )
    return false;
  return true;
}
...
export function validMediaControl(payload: MediaControlPayload): boolean {
  return (
    !!payload &&
    boundedString(payload.id, 100) &&
    ["play", "pause", "seek"].includes(payload.action) &&
    finite(payload.currentTime) &&
