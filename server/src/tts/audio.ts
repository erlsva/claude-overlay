...
export const toneAmountFor = (intensity: "normal" | "shout" | "scream"): number =>
...
export function strainDbFor(intensity: "normal" | "shout" | "scream"): number {
  return intensity === "scream"
    ? SCREAM_STRAIN_DB * screamStrainScale()
    : intensity === "shout"
      ? SHOUT_STRAIN_DB * screamStrainScale()
      : 0;
}
...
export async function renderAudio(opts: {
  id: string;
  scenes: Scene[];
  ...
  const temp = path.join(opts.dataDir, "work", opts.id);
  ...
  try {
    ...
    for (const [index, scene] of opts.scenes.entries()) {
      ...
      let speech: Float32Array | null = null,
        sound: Float32Array | null = null;
      let timing: { start: number; end: number } | null = null;
      if (scene.dialogue.trim()) {
        const raw = path.join(temp, `${index}-speech.mp3`),
          decoded = path.join(temp, `${index}-speech.wav`);
        if (opts.mode === "demo") {
          ...
          await run([
            ...
            "lavfi",
            "-i",
            `sine=frequency=${scene.voice === "voice1" ? 220 : 330}:sample_rate=44100:duration=${length}`,
            "-af",
            "volume=0.2",
            raw,
          ]);
          ...
          const voiceId =
            opts.casting?.find((c) => c.scene === index)?.voiceId || opts.voices[scene.voice];
          ...
          try {
            ...
            throw new Error(
              `Scene ${index + 1} speech generation failed: ${error instanceof Error ? error.message : "ElevenLabs request failed."}`,
            );
          }
          ...
          timing =
            finalWordTiming(payload.normalized_alignment) || finalWordTiming(payload.alignment);
        ...
        await run([
          ...
          "-af",
          channelFilter(scene, {
            pitchShift: !opts.casting?.find((c) => c.scene === index)?.pinned,
            strainDb: strainDbFor(sceneIntensity(scene)),
          }),
          "-ar",
          "44100",
          "-ac",
        ...
        speech = normalizeLoudness(
          screamTone(speech, toneAmountFor(intensity)),
          SPEECH_TARGET_LUFS,
        );
        ...
        if (opts.mode === "elevenlabs" && intensity !== "normal" && layerScale > 0) {
          try {
            opts.progress(
              `Scene ${index + 1}/${opts.scenes.length}: adding ${intensity === "scream" ? "scream" : "shout"} texture`,
            );
            const layerRaw = path.join(temp, `${index}-layer.mp3`),
              layerDecoded = path.join(temp, `${index}-layer.wav`);
            const seconds = Math.min(
              30,
              Math.max(0.5, Math.ceil((speech.length / RATE) * 10) / 10),
            );
            const layerResponse = await eleven("sound-generation", opts.key, {
            ...
            await run([
              ...
              layerRaw,
              "-af",
              "aresample=44100,highpass=f=180,lowpass=f=4000",
              "-ar",
              "44100",
              "-ac",
              "1",
            ...
            speech = normalizeLoudness(
              layerUnderSpeech(
                speech,
                readWav(await readFile(layerDecoded)),
                (intensity === "scream" ? SCREAM_LAYER_GAIN : SHOUT_LAYER_GAIN) * layerScale,
              ),
              SPEECH_TARGET_LUFS,
            );
            ...
            opts.warning?.(
              `Scene ${index + 1}: the ${intensity} texture could not be added (${error instanceof Error ? error.message : "request failed"}), so the voice alone was used.`,
            );
          }
        }
      }
      if (speech && scene.muffled && muffleScale() > 0)
        speech = tameSpikes(
          normalizeLoudness(muffle(speech, muffleScale()), SPEECH_TARGET_LUFS - MUFFLED_LEVEL_LU),
          SHOUTED_SPEECH_SPIKE_LU,
        );
      if (scene.sound.trim()) {
        const raw = path.join(temp, `${index}-sound.mp3`),
          decoded = path.join(temp, `${index}-sound.wav`);
        ...
        if (opts.mode === "demo")
          await run([
            ...
            "lavfi",
            "-i",
            `anoisesrc=color=pink:sample_rate=44100:duration=${soundDuration}`,
            "-af",
            "volume=0.12",
            raw,
          ]);
          ...
          let response: Response;
          try {
            response = await eleven("sound-generation", opts.key, {
              text: soundPrompt,
            ...
            throw new Error(
              `Scene ${index + 1} sound generation failed: ${error instanceof Error ? error.message : "ElevenLabs request failed."}`,
            );
          }
        ...
        sound = normalizeLoudness(
          ...
          (speech ? SPEECH_TARGET_LUFS : SOUND_ONLY_TARGET_LUFS) - (sharp ? 2 : 0),
          24,
          sharp ? 0.45 : 0.6,
        );
        sound = tameSpikes(sound, sharp ? SHARP_SOUND_SPIKE_LU : SOUND_SPIKE_LU);
        if (scene.muffled && !speech && muffleScale() > 0)
          sound = normalizeLoudness(
      ...
      opts.progress(
        `Scene ${index + 1}/${opts.scenes.length}: applying ${scene.channel === "intercom" ? "intercom and " : ""}${scene.effect} / ${scene.duration ?? "natural"}s`,
      );
      let segment: Float32Array;
      try {
        segment = effectTail(speech || sound!, scene, timing, (natural, requested) => {
          opts.warning?.(
        ...
        throw new Error(
          `Scene ${index + 1} audio processing failed: ${error instanceof Error ? error.message : "The scene could not be processed."}`,
        );
      }
      if (speech && sound) {
      ...
      if (!speech && sound)
        segment = softLimit(
          tameSpikes(segment, isSharpSound(scene.sound) ? SHARP_SOUND_SPIKE_LU : SOUND_SPIKE_LU),
          0.72,
          0.5,
        );
