...
const voices: AccountVoice[] = [
  { voice_id: "calm", name: "Calm", labels: { gender: "male", descriptive: "gentle relaxed" } },
  {
    voice_id: "fierce",
    name: "Fierce",
    labels: { gender: "male", descriptive: "fierce intense energetic" },
  },
  {
    voice_id: "bright",
    name: "Bright",
    labels: { gender: "female", descriptive: "playful youthful" },
  },
];
...
test("sound-only scenes do not consume an ElevenLabs voice", () => {
  assert.deepEqual(
    castScenes([scene({ dialogue: "", sound: "several foxes barking" })], voices),
    [],
  );
});
...
test("studio character aliases pin their deliberately created voices", () => {
  const customVoices = [
    {
      voice_id: "generic-fierce",
      name: "Harry - Fierce Warrior",
      labels: { descriptive: "fierce intense energetic" },
    },
  ...
  assert.deepEqual(
    speechRequest(
      scene({
        ...
        delivery: "terrified and breathless",
        prepared: true,
      }),
    ),
    {
...
test("the last word is not respelled and no exclamation stack is added", () => {
  const request = speechRequest(
    scene({ dialogue: "I CAN'T HOLD IT IN", intensity: "scream", prepared: true }),
  );
  assert.equal(request.text, "[screaming] I CAN'T HOLD IT IN!");
});
...
test("a model's long explanation of a scene cannot trigger shouting or become a tag", () => {
  const request = speechRequest(
    scene({
      ...
      character: "man",
      delivery:
        "Performed as a loud, prolonged scream with strong urgency, echo reverberating naturally to fit the cave setting.",
      intensity: "normal",
      prepared: true,
    }),
  );
...
test("legacy plans without an intensity still read the user's short direction", () => {
  assert.equal(
    speechRequest(
      scene({ dialogue: "Aboba", character: "man", delivery: "screaming", prepared: true }),
    ).text,
    "[screaming] ABOBA!",
  );
});
  ...
  const catalog: AccountVoice[] = [
    {
      ...
      labels: { gender: "male", use_case: "characters_animation" },
    },
    {
      voice_id: "warrior",
      name: "Harry - Fierce Warrior",
      labels: { gender: "male", descriptive: "rough", use_case: "characters_animation" },
    },
  ];
  const casting = castScenes(
    [scene({ character: "man", intensity: "scream", preferredVoiceId: "husky" })],
    catalog,
  );
  assert.equal(casting[0].voiceId, "warrior");
  // A calm request may still use the planner's pick.
  assert.equal(
    castScenes([scene({ character: "man", preferredVoiceId: "husky" })], catalog)[0].voiceId,
    "husky",
  );
});
...
const account: AccountVoice[] = [
  {
    ...
    category: "premade",
    labels: { gender: "male", descriptive: "classy" },
  },
  {
    voice_id: "harry",
    name: "Harry - Fierce Warrior",
...
test("character voices are reserved for their own characters", () => {
  ...
  assert.ok(
    !["troll", "pirate"].includes(casting[2].voiceId),
    "plain speech never gets a character voice",
  );
...
test("an account with only character voices can still cast them", () => {
  const casting = castScenes(
    [scene({ character: "someone" })],
    account.filter((voice) => voice.category === "generated"),
  );
...
test("an unhinged character leans toward an energetic voice", () => {
  const casting = castScenes(
    [scene({ character: "schizo man" })],
    [
      {
        voice_id: "calm",
        name: "Calm",
        category: "premade",
        labels: { gender: "male", descriptive: "calm" },
      },
      {
        voice_id: "wild",
        name: "Wild",
        category: "premade",
        labels: { gender: "male", descriptive: "energetic quirky" },
      },
    ],
  );
...
test("how something is said does not pick who says it: a screaming troll gets the troll", () => {
  const catalog: AccountVoice[] = [
    ...
    { voice_id: "c", name: "Angry Pirate", category: "generated" },
    {
      ...
      name: "Harry - Fierce Warrior",
      category: "premade",
      labels: { gender: "male", descriptive: "rough" },
    },
  ];
...
test("welshman and frenchman are men, and their accent is a tag on the line", () => {
  ...
  const [casting] = castScenes(
    [scene({ character: "angry welshman", delivery: "angry welshman" })],
    catalog,
  );
  assert.equal(casting.voiceId, "m");
  const welsh = speechRequest(
    scene({
      character: "angry welshman",
      delivery: "angry welshman",
      dialogue: "[angry] Get off my land.",
    }),
  );
  assert.equal(welsh.text, "[strong Welsh accent] [angry] Get off my land.");
  const french = speechRequest(
    scene({ character: "sad french man", delivery: "sad french man", dialogue: "Je suis triste." }),
  );
  assert.match(french.text, /^\[strong French accent\]/);
  assert.doesNotMatch(
    french.text,
    /french man|welshman/i,
    "the nationality is not repeated as a performance tag",
  );
});
...
test("plain speech is untouched: no accent tag and no nationality guesswork", () => {
  assert.equal(
    speechRequest(scene({ character: "man", delivery: "", dialogue: "Hello there" })).text,
    "Hello there",
  );
  assert.doesNotMatch(
    speechRequest(scene({ dialogue: "I am german in spirit", character: "" })).text,
    /accent/,
  );
});
