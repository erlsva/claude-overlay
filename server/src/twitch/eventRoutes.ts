...
export function createEventRoutes(emitEvent: (type: TriggerEventType, event: any) => void) {
  ...
  router.get("/auth/chatbot/start", (req, res) => {
    const user = getUserFromRequest(req);
    if (!user?.isOwner)
      return res.status(403).json({ error: "Only the overlay owner can connect the chatbot" });
    if (!eventDatabaseConfigured())
      return res.status(503).json({ error: "Event storage is not configured" });
  ...
  router.get("/auth/events/callback", async (req, res) => {
    ...
    try {
      if (req.query.error) {
        console.warn(
          "Twitch Events authorization was denied",
          req.query.error,
      ...
      if (missingScopes.length)
        throw new Error(`Twitch did not grant required scopes: ${missingScopes.join(", ")}`);
  ...
  router.get("/events/status", async (req, res) => {
    ...
    const values = await Promise.all(
      getEventChannels().map(async (channel) => {
    ...
    res.json({
      ...
      chatbot: {
        login: chatbotLogin,
        connected: !!botAuth,
        displayName: botAuth?.displayName,
        scopes: botAuth?.scopes ?? [],
      },
    });
  });
  router.delete("/events/chatbot", async (req, res) => {
    if (!getUserFromRequest(req)?.isOwner)
      return res.status(403).json({ error: "Only the overlay owner can disconnect the chatbot" });
    await deleteEventAuth(CHATBOT_AUTH_KEY);
    res.status(204).end();
  });
  router.delete("/events/:channel", async (req, res) => {
    const channel = req.params.channel as EventChannel;
    if (!isEventChannel(channel) || !canManageEventChannel(req, channel))
      return res.status(403).json({ error: "Not allowed" });
    await deleteEventAuth(channel);
    res.status(204).end();
  });
